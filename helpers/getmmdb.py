...
def mmdbdl():
    r = requests.get("https://download.maxmind.com/geoip/databases/GeoLite2-City/download?suffix=tar.gz", auth=(config.mmdbID, config.mmdbKey)).content
    ...
    if config.OSisWindows: os.system("rd /s /q mmdb && del /f /q mmdb.tar.gz")
